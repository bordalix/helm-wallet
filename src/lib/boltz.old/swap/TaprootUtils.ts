...
import { ScriptElement, Tapleaf } from '../consts/Types'
...
export const tapLeafHash = (leaf: Tapleaf) => liquidTapLeafHash(convertLeaf(leaf))

export const tapBranchHash = (a: Buffer, b: Buffer) => taggedHash('TapBranch/elements', Buffer.concat([a, b]))
...
export const tweakMusig = (musig: Musig, tree: Taptree): Buffer =>
  toXOnly(musig.tweakKey(tapTweakHash(musig.getAggregatedPublicKey(), toHashTree(tree).hash)))

export const createControlBlock = (hashTree: HashTree, leaf: Tapleaf, internalKey: Buffer): Buffer => {
...
export const swapLeafsToTree = (claimLeaf: Tapleaf, refundLeaf: Tapleaf): Taptree => [claimLeaf, refundLeaf]
