...
export const generateAddress = async (wallet: Wallet, index?: number, chain = 1): Promise<any> => {
  ...
  const payment = liquid.payments.p2wpkh({ network, pubkey })
  ...
  return { address: payment.address, blindingKeys, confidentialAddress, pubkey, script }
}
