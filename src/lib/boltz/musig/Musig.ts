...
class Musig {
  ...
  constructor(
    ...
    this.myIndex = this.publicKeys.findIndex((key) => this.key.publicKey.equals(key))
  ...
  public aggregateNonces = (nonces: [Uint8Array, Uint8Array][]) => {
    if (nonces.find(([keyCmp]) => this.key.publicKey.equals(keyCmp)) === undefined) {
  ...
  public verifyPartial = (publicKeyOrIndex: Buffer | number, signature: Uint8Array): boolean => {
    ...
    const publicKey = typeof publicKeyOrIndex === 'number' ? this.publicKeys[publicKeyOrIndex] : publicKeyOrIndex
   ...
   */
  public addPartial = (publicKeyOrIndex: Buffer | number, signature: Uint8Array) => {
    if (!this.verifyPartial(publicKeyOrIndex, signature)) {
      throw 'invalid partial signature'
    }

    this.partialSignatures[this.indexOfPublicKeyOrIndex(publicKeyOrIndex)] = signature
  }
  ...
  public aggregatePartials = (): Buffer => {
    ...
    return Buffer.from(this.secp.musig.partialSigAgg(this.session, this.partialSignatures as Uint8Array[]))
  }

  private indexOfPublicKeyOrIndex = (publicKeyOrIndex: Buffer | number): number => {
    const index =
    ...
    if (index === -1) {
      throw `could not find index of public key ${getHexString(Buffer.from(publicKeyOrIndex as Uint8Array))}`
    }
