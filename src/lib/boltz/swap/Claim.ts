...
import { getHexString, getOutputValue } from '../Utils'
import { OutputType } from '../consts/Enums'
import { scriptBuffersToScript } from './SwapUtils'
import Networks from '../consts/Networks'
import { ClaimDetails, LiquidClaimDetails } from '../consts/Types'
...
export const isRelevantTaprootOutput = (utxo: Omit<ClaimDetails, 'value' | 'keys'>) =>
  utxo.type === OutputType.Taproot && utxo.cooperative !== true

export const validateInputs = (utxos: Omit<ClaimDetails, 'value' | 'keys'>[]) => {
  if (utxos.filter((utxo) => utxo.type !== OutputType.Taproot).some((utxo) => utxo.redeemScript === undefined)) {
...
const getSighashType = (type: OutputType) =>
  type === OutputType.Taproot ? Transaction.SIGHASH_DEFAULT : Transaction.SIGHASH_ALL

const validateLiquidInputs = (utxos: LiquidClaimDetails[], isRefund: boolean) => {
  ...
  if (taprootInputs.some((utxo) => utxo.keys === undefined && utxo.swapTree!.covenantClaimLeaf === undefined)) {
...
export const constructClaimTransaction = (
  ...
  if (utxos.some((utxo) => utxo.type !== OutputType.Taproot && utxo.type !== OutputType.Bech32)) {
    ...
    if (utxo.type === OutputType.Legacy) {
      ...
      updater.addInRedeemScript(
        ...
        scriptBuffersToScript([
          scriptBuffersToScript([getHexString(varuint.encode(ops.OP_0)), crypto.sha256(utxo.redeemScript!)]),
        ]),
      )
    }
  ...
  for (const [i, utxo] of utxos.entries()) {
    if (utxo.type === OutputType.Taproot) {
      ...
      const leafHash = tapLeafHash(isRefund ? utxo.swapTree!.refundLeaf : utxo.swapTree!.claimLeaf)
      const signature = utxo.keys!.signSchnorr(
        pset.getInputPreimage(i, getSighashType(utxo.type), network.genesisBlockHash, leafHash),
      )
    ...
    finalizer.finalizeInput(i, () => {
      ...
      if (utxo.type === OutputType.Taproot) {
        if (utxo.cooperative) {
          ...
          finals.finalScriptWitness = witnessStackToScriptWitness([dummyTaprootSignature])
        } else {
          ...
          finals.finalScriptWitness = witnessStackToScriptWitness(
            witness.concat([
              tapleaf.output,
              createControlBlock(toHashTree(utxo.swapTree!.tree), tapleaf, utxo.internalKey!),
            ]),
          )
        }
      } else if (utxo.type !== OutputType.Legacy) {
        finals.finalScriptWitness = witnessStackToScriptWitness([signatures[i], utxo.preimage, utxo.redeemScript!])
      }
...
const blindPset = (pset: Pset, utxos: LiquidClaimDetails[]) => {
  ...
  const outputBlindingArgs = zkpGenerator.blindOutputs(pset, Pset.ECCKeysGenerator(secp.ecc))
