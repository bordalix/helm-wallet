...
export const p2pkhOutput = (hash: Buffer): Buffer => {
  return script.compile([ops.OP_DUP, ops.OP_HASH160, hash, ops.OP_EQUALVERIFY, ops.OP_CHECKSIG])
}
...
export const p2shOutput = (scriptHex: Buffer): Buffer => {
  return script.compile([ops.OP_HASH160, crypto.hash160(scriptHex), ops.OP_EQUAL])
}
...
export const outputFunctionForType = (type: OutputType): ((input: Buffer) => Buffer) => {
  switch (type) {
